...
class Data:
    ...
    def valid_data(dd, mm, yyyy):
        assert 0 < mm < 13, ValueError('Косяк!')
        m30 = (4, 6, 9, 11)
        if mm in m30:
            assert 0 < dd < 31, ValueError('Косяк!')
        elif mm != 2:
            assert 0 < dd < 32, ValueError('Косяк!')
        elif yyyy % 4 == 0:
            assert 0 < dd < 30, ValueError('Косяк!')
        else:
            assert 0 < dd < 29, ValueError('Косяк!')


dt1 = Data('5 января 91')
dt1.print_dmy_str()

Data.print_data_format()
dt1.print_data_format()
